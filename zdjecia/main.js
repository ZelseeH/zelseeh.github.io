(function () {
  ...
  lb.addEventListener('touchmove', () => {
    touchMoved = true;
  }, { passive: true });

  lb.addEventListener('touchend', e => {
    if (!touchMoved) return; // to był tap, nie swipe – closeLightbox obsłuży klik w tło

    const dx = e.changedTouches[0].clientX - touchStartX;
    const dy = e.changedTouches[0].clientY - touchStartY;

    // ignoruj gdy ruch bardziej pionowy niż poziomy (np. przypadkowy scroll)
